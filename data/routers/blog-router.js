   ...
   if (post) {
     post
       ...
       .catch(err => {
         res
           .status(500)
           .json({ errorMessage: "There was an error retrieving blog post" });
       });
   } else {
     res
       .status(404)
       .json({ errorMessage: "The post with the given id does not exist" });
   }
 });
   ...
   if (post) {
     Blog.findPostComments(id)
       ...
       .catch(err => {
         res
           .status(500)
           .json({ errorMessage: "There was an error retrieving comments" });
       });
   } else {
     res
       .status(404)
 ...
 router.post("/:id/comments", (req, res) => {
   ...
   if (post) {
     if (req.body.text) {
       ...
       res
         .status(500)
         .json({ errorMessage: "Please provide text for your comment" });
     }
   } else {
     res
       .status(404)
